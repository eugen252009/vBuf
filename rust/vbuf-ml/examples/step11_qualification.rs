...
use vbuf_ml::metadata::{encode_payload as encode_metadata, MetadataEntry};
...
fn build_fixture(path: &Path) {
    ...
    let directory = encode_directory(&[TensorEntry::new("layer.0.weight", vec![1], 60, 0)]).unwrap();
    let tokenizer = encode_tokenizer(TokenizerKind::VocabularyOnly, &[
        TokenizerEntry::new(1, true, 31, 0), TokenizerEntry::new(2, true, 32, 0),
        TokenizerEntry::new(3, false, 33, 0), TokenizerEntry::new(4, false, 34, 0),
    ]).unwrap();
    let pool = vec![b'a'; TOKEN_COUNT * TOKEN_BYTES];
    let offsets: Vec<u64> = (0..=TOKEN_COUNT).map(|index| (index * TOKEN_BYTES) as u64).collect();
    ...
    writer.write_opaque(BlockOptions::array(BOOTSTRAP_KEY_ID), &bootstrap).unwrap();
    writer.write_opaque(BlockOptions::array(10), &metadata).unwrap();
    writer.write_opaque(BlockOptions::array(12), &directory).unwrap();
    writer.write_opaque(BlockOptions::array(11), &tokenizer).unwrap();
...
fn faults() -> (u64, u64) {
    ...
    let fields: Vec<&str> = text.rsplit_once(") ").unwrap().1.split_whitespace().collect();
    (fields[7].parse().unwrap(), fields[9].parse().unwrap())
}

fn touch(bytes: &[u8]) -> u64 {
    let mut sum = 0u64;
    for page in bytes.chunks(4096) { sum = sum.wrapping_add(u64::from(page[0])); }
    black_box(sum)
}

fn record(scenario: &str, phase: &str, start: Instant, before: (u64, u64), bytes: u64, checksum: u64) {
    let after = faults();
    println!("{scenario},{phase},{},{},{},{bytes},{checksum}", start.elapsed().as_nanos(), after.0.saturating_sub(before.0), after.1.saturating_sub(before.1));
}
...
fn run(path: &Path, eager: bool) {
    ...
    record(if eager { "eager" } else { "lazy" }, "T1_canonical", start, before, 0, validated.blocks().len() as u64);
    let before = faults(); let start = Instant::now();
    let bootstrap = Bootstrap::discover(&validated).unwrap();
    record(if eager { "eager" } else { "lazy" }, "T2_bootstrap", start, before, bootstrap.regions().iter().map(|region| region.range.length()).sum(), bootstrap.regions().len() as u64);
    let before = faults(); let start = Instant::now();
    ...
    if eager {
        ...
        record("eager", "T4_tokenizer_control", start, before, (TOKEN_COUNT * TOKEN_BYTES + (TOKEN_COUNT + 1) * 8) as u64, tokenizer.token_count());
        let before = faults(); let start = Instant::now();
        let mut checksum = 0u64;
        for index in 0..tokenizer.token_count() { checksum = checksum.wrapping_add(tokenizer.token_text(index).unwrap().len() as u64); checksum = checksum.wrapping_add(tokenizer.score(index).unwrap_or_default() as u64); checksum = checksum.wrapping_add(tokenizer.token_type(index).unwrap_or_default()); }
        record("eager", "T5_tokenizer_payload", start, before, (TOKEN_COUNT * TOKEN_BYTES + TOKEN_COUNT * 4 + TOKEN_COUNT) as u64, checksum);
        let block = validated.block(50, 0).unwrap(); let range = validated.payload_range(validated.blocks().iter().position(|candidate| candidate.block_start == block.block_start).unwrap()).unwrap();
        let before = faults(); let start = Instant::now(); let checksum = touch(range.bytes());
        record("eager", "T6_auxiliary_cold", start, before, range.length(), checksum);
    }
}

fn main() {
    let path = std::env::args().nth(1).map(std::path::PathBuf::from).unwrap_or_else(|| std::env::temp_dir().join("vbuf-ml-step11-qualification.vbuf"));
    build_fixture(&path);
    println!("scenario,phase,nanoseconds,minor_faults,major_faults,explicit_bytes,checksum");
    run(&path, false);
    run(&path, true);
    eprintln!("fixture={} bytes={}", path.display(), fs::metadata(&path).unwrap().len());
}
