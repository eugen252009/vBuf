...
fn real_vbuf_models_expose_validated_consumer_descriptors() {
    for name in ["Qwen3-0.6B-BF16.vbuf", "Qwen3-0.6B-Q8_0.vbuf"] {
        ...
        assert_eq!(
            model.tensor_count().unwrap(),
            if name.contains("BF16") { 311 } else { 310 }
        );
        ...
        assert_eq!(
            model.chat_template().unwrap().unwrap().len(),
            if name.contains("BF16") { 4168 } else { 4100 }
        );
        assert_eq!(
            model.tensor_type(0).unwrap(),
            Some(if name.contains("BF16") {
                ConsumerTensorType::Bf16
            } else {
                ConsumerTensorType::Q8_0
            })
        );
...
fn consumer_descriptor_rejects_missing_files() {
    let path =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../research-models/does-not-exist.vbuf");
