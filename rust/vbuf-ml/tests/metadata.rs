...
use vbuf_ml::metadata::{MetadataEntry, ModelMetadataKey, encode_payload};
...
fn model_file(metadata: &[u8], context: u32) -> Vec<u8> {
    ...
    writer
        ...
        .unwrap();
    writer
        .write_u32(BlockOptions::scalar(21), &[context])
        .unwrap();
...
fn canonical_metadata_fixture_is_self_contained() {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/minimal-model-metadata.vbuf");
    ...
    assert_eq!(
        ModelMetadata::parse(&validated, &bootstrap)
            .unwrap()
            .architecture(),
        Some("llama")
    );
...
fn required_model_metadata_reuses_canonical_values() {
    ...
    assert_eq!(
        metadata.unsigned(ModelMetadataKey::ContextLength),
        Some(2048)
    );
    ...
    assert_eq!(
        metadata
            .get(ModelMetadataKey::ContextLength)
            .unwrap()
            .range
            .bytes()
            .len(),
...
fn optional_unknown_and_required_metadata_rules_are_deterministic() {
    ...
    assert_eq!(
        parse_metadata(&required, 1).unwrap_err(),
...
fn wrong_type_missing_required_and_out_of_range_values_fail() {
    ...
    assert_eq!(
        parse_metadata(&wrong_type, 1).unwrap_err(),
    ...
    .unwrap();
    assert_eq!(
        parse_metadata(&missing, 1).unwrap_err(),
        MlErrorCode::MissingRequiredMetadata
    );
    assert_eq!(
        parse_metadata(&required_metadata(), 0).unwrap_err(),
...
fn malformed_and_duplicate_metadata_are_rejected() {
    ...
    assert_eq!(
        parse_metadata(&metadata[..metadata.len() - 1], 1).unwrap_err(),
