use vbuf_ml::{MetadataEntry, ModelMetadataKey, encode_metadata_payload};

#[test]
fn qwen3_attention_semantics_are_optional_profile_keys() {
    assert_eq!(
        ModelMetadataKey::from_id(9),
        Some(ModelMetadataKey::KVHeadCount)
    );
    assert_eq!(
        ModelMetadataKey::from_id(10),
        Some(ModelMetadataKey::KeyHeadDimension)
    );
    assert_eq!(
        ModelMetadataKey::from_id(11),
        Some(ModelMetadataKey::ValueHeadDimension)
    );
