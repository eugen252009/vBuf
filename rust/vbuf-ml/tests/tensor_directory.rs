...
use vbuf_ml::tensor_directory::{TensorEntry, encode_payload};
use vbuf_ml::{
    Bootstrap, MlErrorCode, TensorDirectory, TensorRepresentation, representation_contract,
};
...
fn parsed(
    directory: &[u8],
    tensors: &[(&[u8], u16, u64)],
) -> Result<TensorDirectory<'static>, MlErrorCode> {
...
fn canonical_fixture_resolves_tensor_directory_through_bootstrap() {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/minimal-tensor-directory.vbuf");
...
fn directory_round_trip_reuses_canonical_primitive_semantics() {
    ...
    assert_eq!(
        tensor_directory.get("scalar").unwrap().dimensions,
        Vec::<u64>::new()
    );
    assert_eq!(
        tensor_directory
...
fn names_are_sorted_and_duplicates_are_rejected() {
    let entries = [
        TensorEntry::new("z", vec![1], 100, 0),
        TensorEntry::new("a", vec![1], 101, 0),
    ];
    let encoded = encode_payload(&entries).unwrap();
    assert_eq!(&encoded[20 + 10..20 + 11], b"a");
    let duplicate = [
        TensorEntry::new("same", vec![1], 100, 0),
        TensorEntry::new("same", vec![1], 101, 0),
    ];
    assert_eq!(
...
fn malformed_truncated_reserved_and_unknown_representation_are_rejected() {
    ...
    assert_eq!(
        parsed(
            truncated,
            &[
                (bytemuck_bytes(&[1.0, 2.0, 3.0, 4.0]), 100, 4),
                (bytemuck_bytes(&[5.0]), 101, 1)
            ]
        )
        .unwrap_err(),
