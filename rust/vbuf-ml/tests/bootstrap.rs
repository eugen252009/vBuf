...
use vbuf_ml::bootstrap::{BOOTSTRAP_KEY_ID, BootstrapEntry, encode_payload};
...
fn stream(entries: &[BootstrapEntry]) -> Vec<u8> {
    ...
    writer
        .write_f32(BlockOptions::array(10), &[1.0, 2.0])
        .unwrap();
    writer
        .write_opaque(BlockOptions::array(11), b"metadata")
        .unwrap();
    writer
        .write_opaque(BlockOptions::array(12), b"tokens")
        .unwrap();
...
fn minimal_bootstrap_maps_roles_to_generic_ranges_without_retyping_values() {
    ...
    assert_eq!(
        bootstrap
            .region(RegionRole::ModelMetadata)
            .unwrap()
            .range
            .bytes(),
        b"metadata"
...
fn unknown_optional_roles_are_skipped_but_unknown_required_roles_fail() {
    ...
    assert_eq!(
        Bootstrap::discover(&parse_v06(&stream(&optional)).unwrap())
            .unwrap()
            .regions()
            .len(),
        3
    ...
    assert_eq!(
        Bootstrap::discover(&parse_v06(&stream(&required)).unwrap())
            .unwrap_err()
            .code,
...
fn missing_duplicate_and_bad_references_fail_deterministically() {
    ...
    assert_eq!(
        Bootstrap::discover(&parse_v06(&stream(&missing)).unwrap())
            .unwrap_err()
            .code,
    ...
    assert_eq!(
        Bootstrap::discover(&parse_v06(&stream(&duplicate)).unwrap())
            .unwrap_err()
            .code,
    ...
    assert_eq!(
        Bootstrap::discover(&parse_v06(&stream(&no_model)).unwrap())
            .unwrap_err()
            .code,
...
fn unsupported_or_truncated_bootstrap_is_an_ml_error_after_canonical_validation() {
    ...
    let validated = parse_v06(&bytes).unwrap();
    assert_eq!(
        Bootstrap::discover(&validated).unwrap_err().code,
