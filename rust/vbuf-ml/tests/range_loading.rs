...
use vbuf_ml::range_loading::{
    Coalescing, PhysicalRange, RangeSource, ReadPlan, execute_plan, select_tensor_names,
};
...
fn model_bytes() -> Vec<u8> {
    ...
    writer
        .write_opaque(BlockOptions::array(90), &[0xaa; 64])
        .unwrap();
    writer.write_f32(BlockOptions::array(21), &[2.0]).unwrap();
    writer
        .write_opaque(BlockOptions::array(91), &[0xbb; 64])
        .unwrap();
...
impl<'a> RangeSource for RecordingSource<'a> {
    ...
    fn read_exact_at(&self, offset: u64, destination: &mut [u8]) -> Result<(), RangeLoadError> {
        ...
        self.calls.borrow_mut().push(PhysicalRange {
            source_id: vbuf_ml::SourceId::SELF,
            offset,
...
fn selected_tensor_set_reads_only_selected_payload_ranges() {
    ...
    assert_eq!(
        plan.targets()
            .iter()
            .map(|target| target.selected.ordinal())
            .collect::<Vec<_>>(),
        vec![2, 0]
    );
...
fn empty_duplicate_and_order_independent_selection_are_safe() {
    ...
    assert_eq!(
        ReadPlan::build(&forward, Coalescing::None).unwrap().reads(),
        ReadPlan::build(&reverse, Coalescing::None).unwrap().reads()
    );
}
...
fn mmap_and_positioned_file_paths_return_identical_selected_bytes() {
    ...
    let mapped_bytes = mapped
        .checked_slice(plan.targets()[0].selected.payload_range())
        .unwrap()
        .to_vec();
...
fn bounds_and_overflow_are_checked() {
    ...
    assert_eq!(
        execute_plan(&source, &plan).unwrap_err(),
        RangeLoadError::OutsideSource
    );
}
