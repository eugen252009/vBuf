...
use vbuf_ml::{
    Bootstrap, BootstrapEntry, LayoutClass, NestedDirectory, NestedEntry, PlacementRequest,
    RegionRole, encode_nested_payload,
};
...
fn inline_child_is_canonically_validated_and_resolved() {
    let child =
        vbuf_ml::write_known_size(Cursor::new(Vec::new()), 3, &[opaque(0x7000, 0, b"child")])
            .unwrap()
            .into_inner();
    let nested = encode_nested_payload(&[NestedEntry {
        name: "child".into(),
        key_id: 0x7000,
        occurrence: 0,
        child_offset: 0,
    ...
    .unwrap();
    let parent = vbuf_ml::write_known_size(
        Cursor::new(Vec::new()),
        3,
        &[
...
fn nested_payload_rejects_empty_ranges() {
    let error = encode_nested_payload(&[NestedEntry {
        name: "empty".into(),
        key_id: 1,
        occurrence: 0,
        child_offset: 0,
