...
use vbuf_ml::{
    SourceDescriptor, SourceId, SourceLocator, SourceRegistry, TensorDirectory, TensorRef,
...
fn artifact(self_size: u64) -> Vec<u8> {
    ...
    let source = SourceDescriptor {
        ...
        locator: SourceLocator::File("authoritative.vbuf".into()),
        hashes: vec![],
    };
    let registry = SourceRegistry::new(vec![
        SourceDescriptor::self_artifact(self_size),
        source.clone(),
    ])
    .unwrap();
    ...
    writer
        .write_opaque(BlockOptions::array(BOOTSTRAP_KEY_ID), &bootstrap_payload)
        .unwrap();
    writer
    ...
    writer
        ...
        .unwrap();
    writer
        .write_block(BlockOptions::array(20), V06Semantic::Float, 32, 0, &[])
        .unwrap();
    writer
        .write_opaque(BlockOptions::array(50), &source_payload)
        .unwrap();
...
fn old_profile_without_source_role_remains_self_compatible() {
    ...
    let bootstrap = Bootstrap::discover(&validated).unwrap();
    assert!(
        parse_source_profile(&validated, &bootstrap)
            .unwrap()
            .is_none()
    );
    let directory = TensorDirectory::parse(&validated, &bootstrap).unwrap();
    assert_eq!(
        directory.get("layer.0").unwrap().payload.source_id(),
        SourceId::SELF
    );
}
