...
use vbuf_ml::layout::{
    LayoutClass, LayoutError, LayoutPlan, PlacementRequest, canonical_payload_alignment,
    payload_shift, write_indefinite, write_known_size,
};
...
fn control_first_order_and_occurrences_are_deterministic() {
    ...
    let bytes = write_known_size(Cursor::new(Vec::new()), 3, &requests)
        .unwrap()
        .into_inner();
...
fn alignment_is_a_payload_refinement_not_a_new_basestep() {
    ...
    let bytes = write_known_size(Cursor::new(Vec::new()), 4, &requests(64))
        .unwrap()
        .into_inner();
    let validated = parse_v06(&bytes).unwrap();
...
fn invalid_alignment_and_ambiguous_order_fail_before_writing() {
    assert_eq!(
        payload_shift(4, 24).unwrap_err(),
        LayoutError::InvalidPayloadAlignment
    );
    assert_eq!(
        payload_shift(4, 8).unwrap_err(),
        LayoutError::InvalidPayloadAlignment
    );
    let mut duplicate = requests(8);
    duplicate[1].order = 2;
    assert_eq!(
        LayoutPlan::build(&duplicate, 3).unwrap_err(),
        LayoutError::DuplicateOrder
    );
}
...
fn known_size_and_indefinite_writers_have_equivalent_blocks() {
    ...
    let known = write_known_size(Cursor::new(Vec::new()), 3, &requests)
        .unwrap()
        .into_inner();
    let indefinite = write_indefinite(Cursor::new(Vec::new()), 3, &requests)
        .unwrap()
        .into_inner();
