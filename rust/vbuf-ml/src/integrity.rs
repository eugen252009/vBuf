...
impl IntegrityEntry {
    pub const fn new(key_id: u16, occurrence: u16, digest: [u8; DIGEST_BYTES]) -> Self {
        Self {
            key_id,
            occurrence,
            digest,
...
impl<'a> IntegrityMetadata<'a> {
    ...
    pub fn discover(
        ...
        let Some(region) = bootstrap.region(RegionRole::IntegrityMetadata) else {
            return Ok(None);
        };
        let block = validated.blocks().get(region.block_index).ok_or_else(|| {
            MlError::new(
                MlErrorCode::IntegrityTargetMissing,
                "integrity metadata block index is absent",
            )
        })?;
            ...
            || block.continuation
        {
            return Err(MlError::new(
                ...
                "integrity metadata must be a non-continuing opaque byte array",
            ));
        }
        ...
        for entry in entries {
            ...
            {
                return Err(MlError::new(
                    if (entry.key_id, entry.occurrence) == (key_id, occurrence) {
                        MlErrorCode::DuplicateIntegrityTarget
                    } else {
                        MlErrorCode::MalformedIntegrityMetadata
                    },
                    "integrity targets must be strictly sorted",
                ));
            }
            ...
            let target_index = validated
                ...
                .ok_or_else(|| {
                    MlError::new(
                        MlErrorCode::IntegrityTargetMissing,
                        "integrity target is absent",
                    )
                })?;
            let target = &validated.blocks()[target_index];
            if target.continuation {
                return Err(MlError::new(
                    MlErrorCode::IntegrityTargetUnsupported,
                    "continuation integrity targets are not supported by profile 0.1",
                ));
            }
    ...
    pub fn records(&self) -> &[IntegrityRecord<'a>] {
        &self.records
    }

    pub fn record(&self, key_id: u16, occurrence: u16) -> Option<&IntegrityRecord<'a>> {
        self.records
            .iter()
            .find(|record| record.key_id == key_id && record.occurrence == occurrence)
    }
    ...
    pub fn verify_target(&self, key_id: u16, occurrence: u16) -> Result<(), MlError> {
        let record = self.record(key_id, occurrence).ok_or_else(|| {
            MlError::new(
                MlErrorCode::NoIntegrityAvailable,
                "no integrity record covers target",
            )
        })?;
        ...
        if actual != record.expected_digest {
            return Err(MlError::new(
                ...
                "integrity digest does not match canonical payload",
            ));
        }
        Ok(())
    }
...
pub fn encode_payload(entries: &[IntegrityEntry]) -> Result<Vec<u8>, MlError> {
    if entries.len() > MAX_INTEGRITY_ENTRIES {
        return Err(MlError::new(
            MlErrorCode::MalformedIntegrityMetadata,
            "integrity entry count exceeds profile maximum",
        ));
    }
    let mut ordered: Vec<&IntegrityEntry> = entries.iter().collect();
    ordered.sort_by_key(|entry| (entry.key_id, entry.occurrence));
    for pair in ordered.windows(2) {
        if (pair[0].key_id, pair[0].occurrence) == (pair[1].key_id, pair[1].occurrence) {
            return Err(MlError::new(
                MlErrorCode::DuplicateIntegrityTarget,
                "integrity target occurs more than once",
            ));
        }
    }
    let total = HEADER_BYTES
        .checked_add(ordered.len().checked_mul(ENTRY_BYTES).ok_or_else(|| {
            MlError::new(
                MlErrorCode::MalformedIntegrityMetadata,
                "integrity size overflows",
            )
        })?)
        .ok_or_else(|| {
            MlError::new(
    ...
    if total > MAX_INTEGRITY_BYTES {
        return Err(MlError::new(
            MlErrorCode::MalformedIntegrityMetadata,
            "integrity metadata exceeds profile maximum",
        ));
    }
    let mut output = Vec::with_capacity(total);
    output.extend_from_slice(&INTEGRITY_MAGIC);
    output.extend_from_slice(&INTEGRITY_VERSION.to_le_bytes());
    ...
    for entry in ordered {
        ...
        output.push(IntegrityAlgorithm::Sha256 as u8);
        output.push(0);
        output.extend_from_slice(&0u16.to_le_bytes());
...
fn parse_payload(bytes: &[u8]) -> Result<Vec<IntegrityEntry>, MlError> {
    if bytes.len() < HEADER_BYTES
        || bytes.len() > MAX_INTEGRITY_BYTES
        || bytes[..8] != INTEGRITY_MAGIC
    {
        return Err(MlError::new(
            MlErrorCode::MalformedIntegrityMetadata,
            "integrity metadata header is invalid",
        ));
    }
    if u16::from_le_bytes(bytes[8..10].try_into().unwrap()) != INTEGRITY_VERSION
    ...
    let count =
        usize::try_from(u32::from_le_bytes(bytes[12..16].try_into().unwrap())).map_err(|_| {
            MlError::new(
                MlErrorCode::MalformedIntegrityMetadata,
                "integrity count exceeds host range",
            )
        })?;
    ...
    if count > MAX_INTEGRITY_ENTRIES || expected != bytes.len() {
        return Err(MlError::new(
            MlErrorCode::MalformedIntegrityMetadata,
            "integrity length does not match entry count",
        ));
    }
    let mut entries = Vec::with_capacity(count);
    for index in 0..count {
        let offset = HEADER_BYTES + index * ENTRY_BYTES;
        if IntegrityAlgorithm::from_id(bytes[offset + 4])? != IntegrityAlgorithm::Sha256
            || bytes[offset + 5] != 0
            || bytes[offset + 6..offset + 8].iter().any(|byte| *byte != 0)
        {
            return Err(MlError::new(
                MlErrorCode::InvalidIntegrityDigest,
                "integrity entry algorithm, flags, or reserved fields are invalid",
            ));
        }
        let mut digest = [0u8; DIGEST_BYTES];
        digest.copy_from_slice(&bytes[offset + 8..offset + 8 + DIGEST_BYTES]);
