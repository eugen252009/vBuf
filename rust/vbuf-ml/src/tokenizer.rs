...
impl TokenizerRole {
    fn from_id(id: u16) -> Option<Self> {
        match id {
            1 => Some(Self::TokenTextBytes),
            2 => Some(Self::TokenOffsets),
            3 => Some(Self::TokenScores),
            4 => Some(Self::TokenTypes),
            5 => Some(Self::BosId),
            6 => Some(Self::EosId),
            7 => Some(Self::UnkId),
            8 => Some(Self::PadId),
            9 => Some(Self::MergeLeftIds),
            10 => Some(Self::MergeRightIds),
            11 => Some(Self::TokenizerModelIdentity),
...
impl TokenizerEntry {
    pub const fn new(role_id: u16, required: bool, generic_key_id: u16, occurrence: u16) -> Self {
        Self {
            role_id,
            required,
            generic_key_id,
    ...
    fn from_value(value: u64) -> Result<Self, MlError> {
        match value {
            ...
            _ => Err(MlError::new(
                ...
                "unsupported tokenizer model identity",
            )),
        }
    }
}
...
impl<'a> TokenizerMetadata<'a> {
    pub fn parse(validated: &ValidatedV06<'a>, bootstrap: &Bootstrap<'a>) -> Result<Self, MlError> {
        let region = bootstrap
            .region(RegionRole::TokenizerMetadata)
            .ok_or_else(|| {
                MlError::new(
                    MlErrorCode::MissingTokenizerReference,
                    "tokenizer metadata role is absent",
                )
            })?;
        let region_block = validated.blocks().get(region.block_index).ok_or_else(|| {
            MlError::new(
                MlErrorCode::MissingTokenizerReference,
                "tokenizer metadata block index is absent",
            )
        })?;
            ...
            || region_block.continuation
        {
            return Err(MlError::new(
                ...
                "tokenizer metadata must be a non-continuing opaque byte array",
            ));
        }
        let (kind, entries) = parse_payload(region.range.bytes())?;
        let mut previous = 0u16;
        let mut seen = [false; ROLE_SLOTS];
        let mut resolved: [Option<(usize, CheckedRange<'a>)>; ROLE_SLOTS] =
            std::array::from_fn(|_| None);
        for (position, entry) in entries.into_iter().enumerate() {
            if position > 0 && entry.role_id <= previous {
                return Err(MlError::new(
                    if entry.role_id == previous {
                        MlErrorCode::DuplicateTokenizerRole
                    } else {
                        MlErrorCode::MalformedTokenizerMetadata
                    },
                    "tokenizer roles must be strictly sorted",
                ));
            }
            previous = entry.role_id;
            let Some(role) = TokenizerRole::from_id(entry.role_id) else {
                if entry.required {
                    return Err(MlError::new(
                        MlErrorCode::MissingTokenizerReference,
                        "unknown required tokenizer role",
                    ));
                }
                continue;
            };
            if entry.required != role.is_required(kind) {
                return Err(MlError::new(
                    MlErrorCode::MalformedTokenizerMetadata,
                    "tokenizer role requiredness is invalid",
                ));
            }
            if seen[entry.role_id as usize] {
                return Err(MlError::new(
                    MlErrorCode::DuplicateTokenizerRole,
                    "tokenizer role occurs more than once",
                ));
            }
            seen[entry.role_id as usize] = true;
            resolved[entry.role_id as usize] = Some(resolve_value(
                validated,
                entry.generic_key_id,
                entry.occurrence,
            )?);
        }
        let text_pool = resolved[TokenizerRole::TokenTextBytes as usize]
            .take()
            .ok_or_else(|| {
                MlError::new(
                    MlErrorCode::MissingTokenizerReference,
                    "token text bytes are missing",
                )
            })?;
        let offsets = resolved[TokenizerRole::TokenOffsets as usize]
            .take()
            .ok_or_else(|| {
                MlError::new(
                    MlErrorCode::MissingTokenizerReference,
                    "token offsets are missing",
                )
            })?;
        validate_text_pool(&validated.blocks()[text_pool.0], text_pool.1.bytes())?;
        let token_count = validate_offsets(
            &validated.blocks()[offsets.0],
            offsets.1.bytes(),
            text_pool.1.bytes(),
        )?;
        if token_count > MAX_VOCABULARY {
            return Err(MlError::new(
                MlErrorCode::TokenizerArrayLengthMismatch,
                "token vocabulary exceeds profile maximum",
            ));
        }
        let scores = optional_array(
            &mut resolved,
            TokenizerRole::TokenScores,
            token_count,
            validated,
            true,
        )?;
        let types = optional_array(
            &mut resolved,
            TokenizerRole::TokenTypes,
            token_count,
            validated,
            false,
        )?;
        ...
        let (merge_left_ids, merge_right_ids) = if matches!(
            ...
            let right = required_u32_array(
                &mut resolved,
                TokenizerRole::MergeRightIds,
                validated,
                MAX_MERGES,
            )?;
            if left.1.bytes().len() != right.1.bytes().len() {
                return Err(MlError::new(
                    MlErrorCode::InvalidMergeTable,
                    "merge arrays have different lengths",
                ));
            }
        ...
        for (role, special) in [
            ...
            (TokenizerRole::EosId, SpecialToken::Eos),
            (TokenizerRole::UnkId, SpecialToken::Unk),
            (TokenizerRole::PadId, SpecialToken::Pad),
        ] {
    ...
    pub fn score_bytes(&self) -> Option<&[u8]> {
        self.scores.as_ref().map(|range| range.bytes())
    }
    /// Borrow optional validated token types.
    pub fn type_bytes(&self) -> Option<&[u8]> {
        self.types.as_ref().map(|range| range.bytes())
    }
    /// Borrow validated little-endian merge-left IDs.
    pub fn merge_left_bytes(&self) -> Option<&[u8]> {
        self.merge_left_ids.as_ref().map(|range| range.bytes())
    }
    /// Borrow validated little-endian merge-right IDs.
    pub fn merge_right_bytes(&self) -> Option<&[u8]> {
        self.merge_right_ids.as_ref().map(|range| range.bytes())
    }
    pub fn specials(&self) -> &[(SpecialToken, u64)] {
        &self.specials
    }
    pub fn model(&self) -> Option<TokenizerModel> {
        self.model
    }
    ...
    pub fn merge_count(&self) -> u64 {
        self.merge_left_ids
            .as_ref()
            .map_or(0, |range| (range.bytes().len() / 4) as u64)
    }
    pub fn merge_pair(&self, index: u64) -> Option<(u64, u64)> {
        ...
        Some((
            u64::from(u32::from_le_bytes(
                left.bytes().get(index..index + 4)?.try_into().ok()?,
            )),
            u64::from(u32::from_le_bytes(
                right.bytes().get(index..index + 4)?.try_into().ok()?,
            )),
        ))
    }
    pub fn chat_template(&self) -> Option<&str> {
        std::str::from_utf8(self.chat_template.as_ref()?.bytes()).ok()
    }
    ...
    pub fn score(&self, index: u64) -> Option<f64> {
        ...
        let block_width = range
            .bytes()
            .len()
            .checked_div(usize::try_from(self.token_count).ok()?)?;
        let start = usize::try_from(index).ok()?.checked_mul(block_width)?;
        match block_width {
            4 => Some(
                f32::from_le_bytes(range.bytes().get(start..start + 4)?.try_into().ok()?) as f64,
            ),
            8 => Some(f64::from_le_bytes(
                range.bytes().get(start..start + 8)?.try_into().ok()?,
            )),
            _ => None,
        }
    }
    ...
    fn offset(&self, index: u64) -> Option<u64> {
        let start = usize::try_from(index).ok()?.checked_mul(8)?;
...
pub fn encode_payload(kind: TokenizerKind, entries: &[TokenizerEntry]) -> Result<Vec<u8>, MlError> {
    if entries.len() > 256 {
        return Err(MlError::new(
            MlErrorCode::MalformedTokenizerMetadata,
            "tokenizer entry count exceeds profile maximum",
        ));
    }
    let mut ordered: Vec<&TokenizerEntry> = entries.iter().collect();
    ordered.sort_by_key(|entry| entry.role_id);
    ...
    output.extend_from_slice(&TOKENIZER_MAGIC);
    output.extend_from_slice(&TOKENIZER_VERSION.to_le_bytes());
    ...
    for entry in ordered {
        output.extend_from_slice(&entry.role_id.to_le_bytes());
...
fn parse_payload(bytes: &[u8]) -> Result<(TokenizerKind, Vec<TokenizerEntry>), MlError> {
    if bytes.len() < HEADER_BYTES
        || bytes.len() > MAX_TOKENIZER_BYTES
        || bytes[..8] != TOKENIZER_MAGIC
    {
        return Err(MlError::new(
            MlErrorCode::MalformedTokenizerMetadata,
            "tokenizer metadata header is invalid",
        ));
    }
    if u16::from_le_bytes(bytes[8..10].try_into().unwrap()) != TOKENIZER_VERSION
    ...
    let expected = HEADER_BYTES
        .checked_add(count.checked_mul(ENTRY_BYTES).ok_or_else(|| {
            MlError::new(
                MlErrorCode::MalformedTokenizerMetadata,
                "tokenizer entry count overflows",
            )
        })?)
        .ok_or_else(|| {
            MlError::new(
                MlErrorCode::MalformedTokenizerMetadata,
                "tokenizer metadata length overflows",
            )
        })?;
    if expected != bytes.len() {
        return Err(MlError::new(
            MlErrorCode::MalformedTokenizerMetadata,
            "tokenizer metadata length does not match entry count",
        ));
    }
    ...
    for index in 0..count {
        ...
        if flags & !1 != 0 || bytes[offset + 8..offset + 12].iter().any(|byte| *byte != 0) {
            return Err(MlError::new(
                MlErrorCode::MalformedTokenizerMetadata,
                "tokenizer entry flags are invalid",
            ));
        }
        entries.push(TokenizerEntry::new(
...
fn resolve_value<'a>(
    ...
    let index = validated
        ...
        .ok_or_else(|| {
            MlError::new(
                MlErrorCode::MissingTokenizerReference,
                "tokenizer canonical reference is absent",
            )
        })?;
    if validated.blocks()[index].continuation {
        return Err(MlError::new(
            MlErrorCode::MissingTokenizerReference,
            "continuation tokenizer arrays are unsupported",
        ));
    }
...
fn validate_text_pool(block: &vbuf_core::v06::V06Block, bytes: &[u8]) -> Result<(), MlError> {
    if block.semantic != V06Semantic::Opaque
        ...
        || block.bit_width != 8
        || u64::try_from(bytes.len()).unwrap_or(u64::MAX) > 64 * 1024 * 1024
    {
        return Err(MlError::new(
            MlErrorCode::MetadataTypeMismatch,
            "token text pool must be canonical byte data",
        ));
    }
    if std::str::from_utf8(bytes).is_err() || bytes.contains(&0) {
        return Err(MlError::new(
            MlErrorCode::InvalidTokenText,
            "token text pool is not valid UTF-8",
        ));
    }
    Ok(())
}
...
fn validate_offsets(
        ...
        || !bytes.len().is_multiple_of(8)
    {
        return Err(MlError::new(
            MlErrorCode::MetadataTypeMismatch,
            "token offsets must be a u64 array",
        ));
    }
    ...
    if first != 0 {
        return Err(MlError::new(
            MlErrorCode::InvalidTokenText,
            "token offsets must start at zero",
        ));
    }
    let pool_len = u64::try_from(pool.len()).unwrap_or(u64::MAX);
    let mut start_offset = first;
    let offset_count = usize::try_from(block.count).map_err(|_| {
        MlError::new(
            MlErrorCode::InvalidTokenText,
            "token offset count exceeds host range",
        )
    })?;
    for index in 1..offset_count {
        let start = index.checked_mul(8).ok_or_else(|| {
            MlError::new(
                MlErrorCode::InvalidTokenText,
                "token offset index overflows",
            )
        })?;
        let end_offset = u64::from_le_bytes(bytes[start..start + 8].try_into().unwrap());
        if end_offset < start_offset
            || end_offset - start_offset > MAX_TOKEN_BYTES
            || end_offset > pool_len
        {
            return Err(MlError::new(
                MlErrorCode::InvalidTokenText,
                "token offset is invalid",
            ));
        }
        let text_start = usize::try_from(start_offset).map_err(|_| {
            MlError::new(
                MlErrorCode::InvalidTokenText,
                "token offset exceeds host range",
            )
        })?;
        let text_end = usize::try_from(end_offset).map_err(|_| {
            MlError::new(
                MlErrorCode::InvalidTokenText,
                "token offset exceeds host range",
            )
        })?;
        let text = pool.get(text_start..text_end).ok_or_else(|| {
            MlError::new(
                MlErrorCode::InvalidTokenText,
                "token offset is outside text pool",
            )
        })?;
        if std::str::from_utf8(text).is_err() || text.contains(&0) {
            return Err(MlError::new(
                MlErrorCode::InvalidTokenText,
                "token text is not valid UTF-8",
            ));
        }
...
fn optional_array<'a>(
    ...
    if block.physical != V06Physical::Array
        || block.count != count
        || (float && !matches!(block.semantic, V06Semantic::Float))
        || (!float && !matches!(block.semantic, V06Semantic::Unsigned))
        || !(float && matches!(block.bit_width, 32 | 64)
            || !float && matches!(block.bit_width, 8 | 16 | 32 | 64))
    {
        return Err(MlError::new(
            MlErrorCode::TokenizerArrayLengthMismatch,
...
fn validate_token_types(types: &CheckedRange<'_>, token_count: u64) -> Result<(), MlError> {
    let count = usize::try_from(token_count).map_err(|_| {
        MlError::new(
            MlErrorCode::InvalidTokenType,
            "token type count exceeds host range",
        )
    })?;
    let width =
        types.bytes().len().checked_div(count).ok_or_else(|| {
            MlError::new(MlErrorCode::InvalidTokenType, "token type array is empty")
        })?;
    for index in 0..count {
        let start = index.checked_mul(width).ok_or_else(|| {
            MlError::new(MlErrorCode::InvalidTokenType, "token type index overflows")
        })?;
        let bytes = types.bytes().get(start..start + width).ok_or_else(|| {
            MlError::new(MlErrorCode::InvalidTokenType, "token type is truncated")
        })?;
        ...
        if value > 6 {
            return Err(MlError::new(
                MlErrorCode::InvalidTokenType,
                "token type is outside pinned llama.cpp domain",
            ));
        }
    }
    Ok(())
...
fn required_unsigned_scalar<'a>(
    resolved: &mut [Option<(usize, CheckedRange<'a>)>; ROLE_SLOTS],
    role: TokenizerRole,
    validated: &ValidatedV06<'a>,
) -> Result<u64, MlError> {
    let (index, range) = resolved[role as usize].take().ok_or_else(|| {
        MlError::new(
            MlErrorCode::MissingTokenizerReference,
            "required tokenizer scalar is absent",
        )
    })?;
    decode_unsigned_scalar(&validated.blocks()[index], range.bytes())
}

fn required_u32_array<'a>(
    resolved: &mut [Option<(usize, CheckedRange<'a>)>; ROLE_SLOTS],
    role: TokenizerRole,
    validated: &ValidatedV06<'a>,
    max_count: u64,
) -> Result<(usize, CheckedRange<'a>), MlError> {
    let value = resolved[role as usize].take().ok_or_else(|| {
        MlError::new(
            MlErrorCode::MissingTokenizerReference,
            "required merge array is absent",
        )
    })?;
    ...
    if block.semantic != V06Semantic::Unsigned
        ...
        || value.1.bytes().len()
            != usize::try_from(block.count.checked_mul(4).ok_or_else(|| {
                MlError::new(
                    MlErrorCode::InvalidMergeTable,
                    "merge array length overflows",
                )
            })?)
            .map_err(|_| {
                MlError::new(
                    MlErrorCode::InvalidMergeTable,
                    "merge array exceeds host range",
                )
            })?
    {
        return Err(MlError::new(
...
fn validate_merge_ids(
    left: &CheckedRange<'_>,
    right: &CheckedRange<'_>,
    token_count: u64,
) -> Result<(), MlError> {
    for index in 0..left.bytes().len() / 4 {
        let offset = index * 4;
        let l = u64::from(u32::from_le_bytes(
            left.bytes()[offset..offset + 4].try_into().unwrap(),
        ));
        let r = u64::from(u32::from_le_bytes(
            right.bytes()[offset..offset + 4].try_into().unwrap(),
        ));
        if l >= token_count || r >= token_count {
            return Err(MlError::new(
                MlErrorCode::InvalidMergeTable,
                "merge ID is outside the vocabulary",
            ));
        }
    }
    Ok(())
...
fn optional_utf8_payload<'a>(
    ...
    if block.semantic != V06Semantic::Opaque
        || block.physical != V06Physical::Array
        || block.bit_width != 8
        || u64::try_from(range.bytes().len()).map_err(|_| {
            MlError::new(
                MlErrorCode::InvalidTokenText,
                "chat template exceeds host range",
            )
        })? > MAX_CHAT_TEMPLATE_BYTES
        || std::str::from_utf8(range.bytes()).is_err()
    {
        return Err(MlError::new(
            MlErrorCode::InvalidTokenText,
            "chat template must be bounded UTF-8 bytes",
        ));
    }
    Ok(Some(range))
}
...
fn decode_unsigned_scalar(block: &vbuf_core::v06::V06Block, bytes: &[u8]) -> Result<u64, MlError> {
        ...
        || !matches!(block.bit_width, 8 | 16 | 32 | 64)
    {
        return Err(MlError::new(
            MlErrorCode::MetadataTypeMismatch,
            "special token must be an unsigned scalar",
        ));
    }
