...
impl TokenizerRole {
    fn from_id(id: u16) -> Option<Self> {
        match id { 1 => Some(Self::TokenTextBytes), 2 => Some(Self::TokenOffsets), 3 => Some(Self::TokenScores), 4 => Some(Self::TokenTypes), 5 => Some(Self::BosId), 6 => Some(Self::EosId), 7 => Some(Self::UnkId), 8 => Some(Self::PadId), _ => None }
    }
    const fn is_required(self) -> bool { matches!(self, Self::TokenTextBytes | Self::TokenOffsets) }
}
...
impl<'a> TokenizerMetadata<'a> {
    pub fn parse(validated: &ValidatedV06<'a>, bootstrap: &Bootstrap<'a>) -> Result<Self, MlError> {
        ...
        let mut resolved: [Option<(usize, CheckedRange<'a>)>; 9] = std::array::from_fn(|_| None);
        for (position, entry) in entries.into_iter().enumerate() {
            ...
            if entry.required != role.is_required() { return Err(MlError::new(MlErrorCode::MalformedTokenizerMetadata, "tokenizer role requiredness is invalid")); }
        ...
        Ok(Self { kind, token_count, text_pool: text_pool.1, offsets: offsets.1, scores, types, specials })
    }
...
fn optional_array<'a>(resolved: &mut [Option<(usize, CheckedRange<'a>)>; 9], role: TokenizerRole, count: u64, validated: &ValidatedV06<'a>, float: bool) -> Result<Option<CheckedRange<'a>>, MlError> {
