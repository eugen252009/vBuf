    ...
    pub fn from_model(
        ...
        if !is_qwen2 && !is_qwen3 {
            return Err(MlError::new(
                ...
                "architecture is not a supported Qwen MoE variant",
            ));
        }
        ...
        if is_qwen2 && (!parameters.shared_experts || parameters.shared_expert_count == 0) {
            return Err(MlError::new(
                ...
                "Qwen2 MoE requires shared expert metadata",
            ));
        }
        let embedding_length = metadata
            ...
            .ok_or_else(|| {
                MlError::new(
                    ...
                    "Qwen MoE embedding length is absent",
                )
            })?;
        let layer_count = metadata
            ...
            .ok_or_else(|| {
                MlError::new(
                    MlErrorCode::MissingRequiredMetadata,
                    "Qwen MoE layer count is absent",
                )
            })?;
        if layer_count != u64::from(parameters.layer_count) {
            return Err(MlError::new(
                MlErrorCode::MoeExpertIndexInvalid,
                "Qwen MoE layer count disagrees with the expert catalog",
            ));
        }
        ...
        for layer in 0..parameters.layer_count {
            ...
            let gate_shape = &[
                embedding_length,
                gate_shape_ff(gate)?,
                u64::from(parameters.expert_count),
            ];
            ...
            if expert_feed_forward_length
                .replace(gate_shape[1])
                .is_some_and(|length| length != gate_shape[1])
            {
                return Err(MlError::new(
                    MlErrorCode::TensorRepresentationMismatch,
                    "Qwen expert feed-forward dimensions differ between layers",
                ));
            }
            if is_qwen2 {
                for suffix in [
        ...
        let expert_feed_forward_length = expert_feed_forward_length.ok_or_else(|| {
            MlError::new(
                MlErrorCode::MoeExpertReferenceMissing,
                "Qwen MoE has no expert layers",
            )
        })?;
        Ok(Self {
            architecture,
...
fn tensor<'a>(
    ...
    tensors.get(name).ok_or_else(|| {
        MlError::new(
            MlErrorCode::MoeExpertReferenceMissing,
            "Qwen MoE tensor is missing",
        )
    })
}

fn gate_shape_ff(tensor: &crate::TensorDescriptor<'_>) -> Result<u64, MlError> {
    if tensor.dimensions.len() != 3 || tensor.dimensions[1] == 0 {
        return Err(MlError::new(
            MlErrorCode::TensorRepresentationMismatch,
            "Qwen expert tensor rank or dimension is invalid",
        ));
    }
...
fn require_shape(tensor: &crate::TensorDescriptor<'_>, expected: &[u64]) -> Result<(), MlError> {
    if tensor.dimensions != expected {
        return Err(MlError::new(
            MlErrorCode::TensorRepresentationMismatch,
            "Qwen MoE tensor shape does not match the architecture contract",
        ));
    }
    Ok(())
}
