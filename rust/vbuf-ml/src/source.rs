...
pub fn encode_profile(
    ...
    for descriptor in &registry.descriptors {
        descriptor.validate().map_err(|_| {
            MlError::new(
                MlErrorCode::InvalidSourceLocator,
                "source descriptor is malformed",
            )
        })?;
        ...
        let size = descriptor.declared_size.ok_or_else(|| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "persistent source size is required",
            )
        })?;
        let locator_len = u32::try_from(locator.len()).map_err(|_| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "source locator is too large",
            )
        })?;
        let hash_count = u16::try_from(descriptor.hashes.len()).map_err(|_| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "source hash count is too large",
            )
        })?;
        ...
        for hash in &descriptor.hashes {
            let length = u16::try_from(hash.value.len()).map_err(|_| {
                MlError::new(MlErrorCode::InvalidSourceHash, "source hash is too large")
            })?;
    ...
    for (key_id, occurrence, reference) in bindings {
        if seen.contains(&(*key_id, *occurrence)) {
            return Err(MlError::new(
                MlErrorCode::DuplicateSourceBinding,
                "source binding occurs more than once",
            ));
        }
        seen.push((*key_id, *occurrence));
        if registry.get(reference.source_id()).is_none() {
            return Err(MlError::new(
                MlErrorCode::UnknownRequiredRole,
                "source binding references an unknown source",
            ));
        }
    ...
    if output.len() > MAX_SOURCE_METADATA_BYTES {
        return Err(MlError::new(
            MlErrorCode::MalformedSourceMetadata,
            "source metadata exceeds bounded size",
        ));
    }
    Ok(output)
}
...
pub fn parse_profile(
    _validated: &ValidatedV06<'_>,
    bootstrap: &Bootstrap<'_>,
) -> Result<Option<PersistentSourceMetadata>, MlError> {
    let Some(region) = bootstrap.region(RegionRole::SourceMetadata) else {
        return Ok(None);
    };
    let bytes = region.range.bytes();
    if bytes.len() < SOURCE_HEADER_BYTES
        || bytes.len() > MAX_SOURCE_METADATA_BYTES
        || bytes[..8] != SOURCE_METADATA_MAGIC
    {
        return Err(MlError::new(
            MlErrorCode::MalformedSourceMetadata,
            "source metadata header is invalid",
        ));
    }
    if u16::from_le_bytes(bytes[8..10].try_into().unwrap()) != SOURCE_METADATA_VERSION {
        return Err(MlError::new(
            MlErrorCode::UnsupportedSourceMetadataVersion,
    ...
    if bytes[10..12].iter().any(|byte| *byte != 0) || bytes[20..24].iter().any(|byte| *byte != 0) {
        return Err(MlError::new(
            MlErrorCode::MalformedSourceMetadata,
            "source metadata reserved fields are non-zero",
        ));
    }
    let source_count = usize::try_from(u32::from_le_bytes(bytes[12..16].try_into().unwrap()))
        .map_err(|_| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
    ...
    let binding_count = usize::try_from(u32::from_le_bytes(bytes[16..20].try_into().unwrap()))
        .map_err(|_| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
    ...
    for _ in 0..source_count {
        let header = bytes
            .get(cursor..cursor + SOURCE_ENTRY_BYTES)
            .ok_or_else(|| {
                MlError::new(
                    MlErrorCode::MalformedSourceMetadata,
                    "source entry is truncated",
                )
            })?;
        let id = SourceId::new(u64::from_le_bytes(header[..8].try_into().unwrap()));
        let size = u64::from_le_bytes(header[8..16].try_into().unwrap());
        let kind = header[16];
        if header[17..20].iter().any(|byte| *byte != 0)
            || header[26..32].iter().any(|byte| *byte != 0)
        {
            return Err(MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "source entry reserved fields are non-zero",
            ));
        }
        let locator_len = usize::try_from(u32::from_le_bytes(header[20..24].try_into().unwrap()))
            .map_err(|_| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "source locator length is invalid",
            )
        })?;
        let hash_count = usize::from(u16::from_le_bytes(header[24..26].try_into().unwrap()));
        cursor += SOURCE_ENTRY_BYTES;
        let locator_bytes = bytes.get(cursor..cursor + locator_len).ok_or_else(|| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "source locator is truncated",
            )
        })?;
        cursor += locator_len;
        let locator = match kind {
            0 if locator_bytes.is_empty() => SourceLocator::SelfArtifact,
            1 => SourceLocator::File(String::from_utf8(locator_bytes.to_vec()).map_err(|_| {
                MlError::new(
                    MlErrorCode::InvalidSourceLocator,
                    "file locator is not UTF-8",
                )
            })?),
            2 => SourceLocator::Http(String::from_utf8(locator_bytes.to_vec()).map_err(|_| {
                MlError::new(
                    MlErrorCode::InvalidSourceLocator,
                    "HTTP locator is not UTF-8",
                )
            })?),
            _ => {
                return Err(MlError::new(
                    MlErrorCode::InvalidSourceLocator,
                    "source locator kind is unsupported",
                ));
            }
        };
        let mut hashes = Vec::with_capacity(hash_count);
        for _ in 0..hash_count {
            let header = bytes.get(cursor..cursor + 4).ok_or_else(|| {
                MlError::new(
                    MlErrorCode::MalformedSourceMetadata,
                    "source hash header is truncated",
                )
            })?;
            ...
            let value = bytes
                ...
                .ok_or_else(|| {
                    MlError::new(MlErrorCode::InvalidSourceHash, "source hash is truncated")
                })?
                .to_vec();
            cursor += length;
    ...
    let registry = SourceRegistry::new(descriptors).map_err(|_| {
        MlError::new(
            MlErrorCode::MalformedSourceMetadata,
    ...
    for _ in 0..binding_count {
        let entry = bytes.get(cursor..cursor + BINDING_BYTES).ok_or_else(|| {
            MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "source binding is truncated",
            )
        })?;
        if entry[4..8].iter().any(|byte| *byte != 0) {
            return Err(MlError::new(
                MlErrorCode::MalformedSourceMetadata,
                "source binding reserved fields are non-zero",
            ));
        }
        ...
        let reference = registry
            .checked_range(source_id, offset, length)
            .map_err(|_| {
                MlError::new(
                    MlErrorCode::MalformedSourceMetadata,
                    "source binding range is invalid",
                )
            })?
            .tensor_ref();
    ...
    if cursor != bytes.len() {
        return Err(MlError::new(
            MlErrorCode::MalformedSourceMetadata,
            "source metadata has trailing bytes",
        ));
    }
