...
fn validate_target(path: &Path, source: &[u8], plan: &Plan, evidence: &Path) -> Result<(), String> {
    ...
    if plan.moe.is_some() { let nested = vbuf_ml::NestedDirectory::parse(&validated, &bootstrap).map_err(|e| e.to_string())?; let moe = vbuf_ml::MoeDirectory::parse(&validated, &bootstrap, &nested).map_err(|e| e.to_string())?; if metadata.architecture().unwrap_or("") != "qwen35moe" { vbuf_ml::DeepSeekMoELoader::from_model(metadata.architecture().unwrap_or(""), &metadata, &directory, &moe).map_err(|e| e.to_string())?; } }
