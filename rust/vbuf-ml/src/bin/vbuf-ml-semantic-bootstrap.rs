...
use vbuf_ml::{
    Bootstrap, BorrowedModelView, ModelMetadata, RegionRole, SourceDescriptor, SourceHash,
    SourceId, SourceLocator, SourceRegistry, TensorDirectory, TensorRef, TokenizerMetadata,
    encode_source_profile, parse_source_profile,
};
...
fn build(source_bytes: &[u8], metadata_size: u64, locator: &str) -> Result<Vec<u8>, String> {
    ...
    let bindings: Vec<(u16, u16, TensorRef)> = directory
        ...
        .map(|tensor| {
            ...
            .map_err(|e| format!("tensor source reference: {e}"))
        })
        .collect::<Result<_, _>>()?;
    ...
    let mut profile_entries: Vec<BootstrapEntry> = bootstrap
        ...
        .collect();
    profile_entries.push(BootstrapEntry::new(
    ...
    for (index, block) in validated.blocks().iter().enumerate() {
        if index == original_bootstrap_index {
            ...
            if block.physical != V06Physical::Array {
                return Err("tensor placeholder is not an array".into());
            }
            writer
                .write_block(
...
fn main() {
    ...
    let target_path = PathBuf::from(
        std::env::args()
            .nth(2)
            .expect("semantic bootstrap output path"),
    );
    let locator = std::env::args()
        .nth(3)
    ...
    assert_eq!(
        first.len(),
        output.len(),
        "self-source size changed profile geometry"
    );
    ...
    let target_validated =
        parse_v06(&target_mapping).expect("semantic bootstrap canonical validation");
    let target_bootstrap =
        Bootstrap::discover(&target_validated).expect("semantic bootstrap profile");
    let profile = parse_source_profile(&target_validated, &target_bootstrap)
        .expect("source profile parse")
        .expect("source profile role");
    BorrowedModelView::parse_with_sources(&target_mapping, &profile.registry, &[])
        .expect("semantic bootstrap semantic parse");
    println!(
        "source_bytes={} bootstrap_bytes={} target={}",
        mapping.len(),
        output.len(),
        target_path.display()
    );
