...
int main(int argc, char ** argv) {
    ...
    const bool direct = std::strcmp(argv[1], "direct") == 0;
    ...
    llama_model * model = direct ? llama_model_load_vbuf_direct(argv[2], params) : (vbuf ? llama_model_load_vbuf(argv[2], params) : llama_model_load_from_file(argv[2], params));
    auto end = std::chrono::steady_clock::now();
    if (!model) return 1;
    std::printf("{\"format\":\"%s\",\"phase\":\"model_ready\",\"duration_us\":%.3f}\n", argv[1], std::chrono::duration<double, std::micro>(end-start).count());
    if (direct) llama_model_free_vbuf_direct(model); else if (vbuf) llama_model_free_vbuf(model); else llama_model_free(model); llama_backend_free(); return 0;
}
