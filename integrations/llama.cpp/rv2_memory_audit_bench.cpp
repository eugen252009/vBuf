...
int main(int argc, char ** argv) {
    ...
    llama_model * model = is_vbuf ? llama_model_load_vbuf_direct(path, model_params) : llama_model_load_from_file(path, model_params);
    ...
    if (n_prompt < 0) { prompt_tokens.resize(static_cast<size_t>(-n_prompt)); n_prompt = llama_tokenize(vocab, prompt, static_cast<int32_t>(std::strlen(prompt)), prompt_tokens.data(), -n_prompt, true, false); }
    ...
    vbuf_bench::event("PROMPT_EVAL_END", "success");
    const int32_t n_vocab = llama_vocab_n_tokens(vocab);
    llama_token token = greedy(llama_get_logits_ith(context, -1), n_vocab);
    for (int i = 0; i < 3; ++i) {
        batch = llama_batch_get_one(&token, 1); batch.logits = logits.data();
        if (llama_decode(context, batch) != 0) return 7;
        token = greedy(llama_get_logits_ith(context, -1), n_vocab);
    }
    vbuf_bench::event("BEFORE_CONTEXT_FREE");
    llama_free(context);
    vbuf_bench::event("AFTER_CONTEXT_FREE");
    vbuf_bench::event("BEFORE_MODEL_FREE");
    if (is_vbuf) llama_model_free_vbuf_direct(model); else llama_model_free(model);
    vbuf_bench::event("AFTER_MODEL_FREE");
    usleep(500000);
    llama_backend_free();
    vbuf_bench::event("AFTER_BACKEND_FREE");
    usleep(500000);
    return 0;
}
