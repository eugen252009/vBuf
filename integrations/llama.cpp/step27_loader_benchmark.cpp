...
int main(int argc, char ** argv) {
    ...
    std::printf("],\"spans\":[");
    for (size_t i = 0; i < preparation.spans.size(); ++i) { const auto & span = preparation.spans[i]; std::printf("%s{\"role\":\"%s\",\"label\":\"%s\",\"layer_id\":%llu,\"start_offset\":%llu,\"end_offset\":%llu,\"span_bytes\":%llu,\"useful_bytes\":%llu,\"gap_bytes\":%llu,\"tensor_count\":%llu}", i ? "," : "", span.role.c_str(), span.label.c_str(), (unsigned long long) span.layer_id, (unsigned long long) span.start_offset, (unsigned long long) span.end_offset, (unsigned long long) span.span_bytes, (unsigned long long) span.useful_bytes, (unsigned long long) span.gap_bytes, (unsigned long long) span.tensor_count); }
    std::printf("]},\"phases\":{");
