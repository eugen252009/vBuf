...
def main() -> int:
    ...
    try:
        subprocess.run(["cargo","test","--manifest-path",str(root/"rust/Cargo.toml"),"-p","vbuf-ml","--test","consumer","--test","consumer_ffi"],cwd=root,check=True)
    ...
    metadata=[]; tensors=[]
    values=[("Architecture","qwen3"),("ContextLength",40960),("EmbeddingLength",1024),("LayerCount",28),("HeadCount",16),("KVHeadCount",8),("KeyHeadDimension",128),("ValueHeadDimension",128),("FeedForwardLength",3072),("NormalizationEpsilon",1e-6),("RopeTheta",1000000.0)]
    ...
    (out/"adapter-provenance.json").write_text(json.dumps({"repository":"https://github.com/ggml-org/llama.cpp.git","base_commit":PINNED,"checkout":"external pinned worktree (path supplied at runtime)","integration":"validated Rust C ABI + C++ descriptor wrapper (runtime qualification is separate)","source_locations":{"model_entry":"src/llama.cpp:377-425","loader":"src/llama-model-loader.cpp:519-560","model_create":"src/llama-model.cpp:344-350","load_vocab":"src/llama-model.cpp:1261-1265","load_tensors":"src/llama-model.cpp:1267-1270","create_tensor":"src/llama-model-loader.cpp:1060-1320","load_data":"src/llama-model-loader.cpp:1390-1411","output_fallback":"src/llama-model-loader.cpp:1101-1107"},"status":{"descriptor_bridge":"PASS","tokenizer_descriptor_bridge":"PASS","llama_model_vbuf_construction":"DEFERRED","tokenizer_parity":"DEFERRED","logit_parity":"DEFERRED","generation_parity":"DEFERRED"}},indent=2)+"\n")
