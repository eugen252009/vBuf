...
exec >> "$artifact_dir/reference.log" 2>&1
set +e
python -u "$root_dir/research/results/vbuf-ml-integration/step32g_bounded_reference.py" \
    "$manifest" \
    "$root_dir/.step32c/glm-4.5-air-fp8.vbuf" \
    "$checkpoint" \
    46 generation Test "$1"
status=$?
printf 'REFERENCE_EXIT_STATUS=%s\n' "$status"
date -Is > "$artifact_dir/reference-completed-at.tmp"
printf 'EXIT_STATUS=%s\n' "$status" >> "$artifact_dir/reference-completed-at.tmp"
mv "$artifact_dir/reference-completed-at.tmp" "$artifact_dir/reference-completed-at"
exit "$status"
